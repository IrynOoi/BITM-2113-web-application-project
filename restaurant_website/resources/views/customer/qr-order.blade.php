...
@section('content')
        ...
        <div class="container">
            <div class="table-info-bar">
                <div class="table-info-left">
                    ...
                    <div><span class="table-label">Table</span><span class="table-number" id="displayTableNumber">1</span>
                    </div>
                </div>
                <div class="table-info-center"><i class="fas fa-users"></i><span id="displayPax">1 pax</span></div>
                <div class="table-info-right"><button class="btn-change-table" id="btnChangeTable"><i
                            class="fas fa-exchange-alt"></i> Change</button></div>
            </div>
            ...
            <div class="no-results" id="noResults" style="display: none;"><i class="fas fa-utensils"></i>
                <p>No items found</p>
            </div>
        </div>
    </main>
    ...
    <div class="order-summary-bar" id="orderSummaryBar" style="display: none;">
        <div class="order-summary-content">
            <div class="order-summary-left" id="btnToggleOrder">
                <div class="order-icon"><i class="fas fa-receipt"></i><span class="order-count-badge"
                        id="orderCountBadge">0</span></div>
                <div class="order-info"><span class="order-label">Your Order</span><span class="order-total"
                        id="orderTotal">RM 0.00</span></div>
                <i class="fas fa-chevron-up order-toggle-icon" id="orderToggleIcon"></i>
            </div>
            <button class="btn-place-order" id="btnPlaceOrder" disabled><i class="fas fa-paper-plane"></i> Place
                Order</button>
        </div>
        <div class="order-items-panel" id="orderItemsPanel" style="display: none;">
            <div class="order-items-header">
                <h3>Order Details</h3><button class="btn-clear-order" id="btnClearOrder"><i class="fas fa-trash-alt"></i>
                    Clear All</button>
            </div>
            <div class="order-items-list" id="orderItemsList"></div>
            <div class="order-empty" id="orderEmpty"><i class="fas fa-clipboard-list"></i>
    ...
    <div class="modal-overlay" id="successModal" style="display: none;">
        <div class="modal-card success-modal-card">
            ...
            <div class="modal-actions">
                <a href="{{ route('customer.order-status') }}" class="btn-primary"><i class="fas fa-eye"></i> Track
                    Order</a>
    ...
    <form id="qrSubmitForm" method="POST" action="{{ route('customer.orders.store') }}" style="display:none;">
        @csrf
        <input type="hidden" name="custName" id="hiddenCustName">
        <input type="hidden" name="custPhone" id="hiddenCustPhone">
        <input type="hidden" name="orderType" value="dine-in">
        <input type="hidden" name="tableNumber" id="hiddenTableNumber">
        <input type="hidden" name="paxNumber" id="hiddenPaxNumber">
...
@section('scripts')
    <script>
        @php
            $trackParams = ['qr' => 1];
            if (request('table')) $trackParams['table'] = request('table');
            if (request('pax'))   $trackParams['pax']   = request('pax');
        @endphp
        window.orderStatusUrl = @json(route('customer.order-status', $trackParams));
    </script>
