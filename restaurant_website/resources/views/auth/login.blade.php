...
@section('title', 'Login - Restoran SUP TULANG ZZ')
...
<style>
    ...
    .role-hint { display: flex; align-items: center; gap: 8px; background: #fff3cd; border: 1px solid #ffc107; border-radius: 8px; padding: 10px 14px; font-size: 0.82rem; color: #856404; margin-bottom: 20px; }
    ...
    .error-message { font-size: 0.78rem; color: #e74c3c; }
    .remember-row { display: flex; align-items: center; justify-content: space-between; }
    .checkbox-label { display: flex; align-items: center; gap: 7px; font-size: 0.875rem; cursor: pointer; }
    .checkbox-label input[type="checkbox"] { accent-color: #c0392b; width: 15px; height: 15px; }
    .forgot-link { font-size: 0.85rem; color: #c0392b; text-decoration: none; }
            ...
            <form class="auth-form" method="POST" action="{{ route('login') }}">
                ...
                <div class="form-group">
                    ...
                    <div class="input-wrapper">
                        ...
                        <input type="email" id="email" name="email" value="{{ old('email') }}" placeholder="your@email.com" required autofocus autocomplete="username">
                    </div>
                </div>
