...
<body>
    ...
    <header class="header">
        ...
        <nav class="desktop-nav" id="desktopNav">
            <ul>
                ...
                <li><a href="{{ url('news-events') }}" class="{{ request()->is('news-events') ? 'active' : '' }}">News &amp; Events</a></li>
                <li><a href="{{ url('about') }}" class="{{ request()->is('about') ? 'active' : '' }}">About</a></li>
                <li><a href="{{ url('contact') }}" class="{{ request()->is('contact') ? 'active' : '' }}">Contact</a></li>
                @auth
                    <li><a href="{{ Auth::user()->role === 'customer' ? route('customer.dashboard') : route('staff.dashboard') }}" class="btn-login">
                        <i class="fas fa-user"></i> {{ Auth::user()->name ?? Auth::user()->full_name ?? 'Dashboard' }}
                    </a></li>
                    <li>
                        <form method="POST" action="{{ route('logout') }}" style="display:inline;">
                            @csrf
                            <a href="{{ route('logout') }}" class="btn-register" onclick="event.preventDefault(); this.closest('form').submit();">Logout</a>
                        </form>
                    </li>
                @else
                    <li><a href="{{ route('login') }}" class="btn-login {{ request()->routeIs('login') ? 'active' : '' }}">Login</a></li>
        ...
        <div class="container">
            <div class="footer-grid">
                <div class="footer-section">
                    <h3><img src="{{ asset('assets/images/Footer-logo.jpeg') }}" alt="Logo" style="height:50px;width:auto;vertical-align:middle;"> Restoran SUP TULANG ZZ</h3>
                ...
                <div class="footer-section">
                    ...
                    <ul class="footer-links">
                        ...
                        <li><a href="{{ url('news-events') }}"><i class="fas fa-chevron-right"></i> News &amp; Events</a></li>
    ...
    <nav class="mobile-nav">
        ...
        @auth
            <a href="{{ Auth::user()->role === 'customer' ? route('customer.dashboard') : route('staff.dashboard') }}"><i class="fas fa-user"></i><span>{{ Auth::user()->name ?? Auth::user()->full_name ?? 'Profile' }}</span></a>
        @else
